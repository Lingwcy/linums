...
import { createChatCompletionReader, inferProviderIdFromModelId, type ProviderId } from '@/server/services/ai/gateway'
...
export async function handleChatRequest(
  ...
  // 6. 调用 AI API
  const providerId = requestedProviderId ?? inferProviderIdFromModelId(model)
  const { reader } = await createChatCompletionReader(providerId, apiKey, {
    model,
