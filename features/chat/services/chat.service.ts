...
export const ChatService = {
  ...
  async sendMessage(
    ...
    if (!store.selectedModel) {
      toast({
        title: '未选择模型',
        description: '请先在设置中添加模型，然后在模型选择器中选择。',
      })
      return
    }
    ...
    try {
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          content,
          conversationId,
          model: store.selectedModel, // 选中的模型ID
          providerId: store.selectedProviderId, // 选中的供应商ID
          modelInfo: store.selectedModelInfo, // 选中的模型信息
